...
use Project\Issue\Priority;
use Project\Issue\Status;
use User\Activity;
...
class Issue extends \Eloquent
{
	...
	const ISSUE_TAG_STATUS_OPENED      = 1;
	const ISSUE_TAG_STATUS_CLOSED      = 2;
	const ISSUE_TAG_TYPE_FEATURE       = 3;
	const ISSUE_TAG_TYPE_BUG           = 4;
	const ISSUE_TAG_RESOLUTION_WONTFIX = 6;
	const ISSUE_TAG_RESOLUTION_FIXED   = 7;
	const ISSUE_TAG_STATUS_TESTING     = 8;
	const ISSUE_TAG_STATUS_TOFIX       = 9;
	const ISSUE_TAG_STATUS_PRODUCTION  = 10;
	...
	/**
	 * @return \Project\Issue\Status
	 */
	public function status()
	{
		return $this->belongs_to('Project\Issue\Status');
	}

	/**
	 * @return \Project\Issue\Priority
	 */
	public function priority()
	{
		return $this->belongs_to('Project\Issue\Priority');
	}
	...
	public function activity( $activity_limit = 5 )
	{
		...
		foreach (\User\Activity::where('item_id', '=', $issue->id)->order_by('created_at', 'DESC')->get() as $activity)
		{
			$activities[] = $activity;
		...
		foreach ($activities as $row)
		{
			switch ($row->type_id)
			{
				case 2:
					...
					$return[] = \View::make(
						'project/issue/activity/' . $activity_type[$row->type_id]->activity, array(
							                                                                   'issue'    => $issue,
							                                                                   'project'  => $project,
				...
				9:
					...
					$return[] = \View::make(
						'project/issue/activity/' . $activity_type[$row->type_id]->activity, array(
							                                                                   ...
							                                                                   'user'     => $users[$row->user_id],
							                                                                   'activity' => $row,
							                                                                   'priority' => Priority::find($row->action_id),
						                                                                   )
					);
	 ...
	 * @return void
	 */
	public function prioritize( $priority_id )
	{
		$this->priority_id = $priority_id;
		$this->save();

		\User\Activity::add(Activity::CHANGE_PRIORITY, $this->project_id, $this->id, $priority_id, null);
	}
	...
	public function change_status( $status )
	{
		if ($status == Status::STATUS_CLOSED)
		{
			...
			/* Add to activity log */
			\User\Activity::add(\User\Activity::CLOSE_ISSUE, $this->project_id, $this->id);
		}
		elseif ($status == Status::STATUS_TESTING) // Testing
		{
			/* Add to activity log */
			\User\Activity::add(\User\Activity::TESTING_ISSUE, $this->project_id, $this->id);
		}
		elseif ($status == Status::STATUS_RESOLVING) // Production
		{
			/* Add to activity log */
			\User\Activity::add(\User\Activity::RESOLVING_ISSUE, $this->project_id, $this->id);
		}
		else
		{
			/* Add to activity Log */
			\User\Activity::add(\User\Activity::REOPEN_ISSUE, $this->project_id, $this->id);
		}
		...
		if ($this->assigned_to && $this->assigned_to != \Auth::user()->id)
		{
			...
			$verb    = ($this->status_id == Status::STATUS_CLOSED ? __('email.closed') : __('email.reopened'));
			$subject = sprintf(__('email.issue_changed'), $this->title, $project->name, $verb);
			$text    = \View::make(
				'email.change_status_issue', array(
					                           'actor'   => \Auth::user()->firstname . ' ' . \Auth::user()->lastname,
					                           'project' => $project,
					                           'issue'   => $this,
					                           'verb'    => $verb
	...
	public function set_tags( $mode )
	{
		...
		if ($has_tag_diff)
		{
			...
			\User\Activity::add(
				6, $this->project_id, $this->id, null, json_encode(
					 array(
	...
	public static function create_issue( $input, $project )
	{
		...
		$fill = array(
			...
			'title'       => $input['title'],
			'body'        => $input['body'],
			'status_id'   => Status::STATUS_OPENED,
		...
		\User\Activity::add(Activity::CREATE_ISSUE, $project->id, $issue->id);
		...
		if ($issue->assigned_to)
		{
			if ($issue->assigned_to != \Auth::user()->id)
			{
				...
				$text    = \View::make(
					'email.new_assigned_issue', array(
						                          'project' => $project,
						                          'issue'   => $issue,
					                          )
				);
	...
	public static function count_issues()
	{
		...
		$count       = \DB::table('projects_issues')
			...
			->where('projects_issues.status_id', '=', Status::STATUS_OPENED)
			->count();
		...
		$count                      = \DB::table('projects_issues')
			->join('projects', 'projects.id', '=', 'projects_issues.project_id')
			->where('projects.status', '=', 1)
			->where('projects_issues.status_id', '=', Status::STATUS_CLOSED)
			->count();
